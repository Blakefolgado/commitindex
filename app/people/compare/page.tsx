...
const peopleIndex = (recentlySearchedData as { entries: PersonSummary[] }).entries;
...
export default async function ComparePeople({
  ...
  const people = wanted
    .map((login) => peopleIndex.find((entry) => entry.login.toLowerCase() === login))
    .filter((entry): entry is PersonSummary => Boolean(entry?.months?.length));
